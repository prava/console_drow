...
abstract class WebSocketServer {
  ...
  public function run() {
    ...
    while(true) {
      ...
      $this->_tick();
      $this->tick();
      @socket_select($read,$write,$except,0, 1000);
...
//echo "ID=".$ID."\n";
//print_r($user);
		  $query = "SELECT * FROM points WHERE rec_id > '".$user->lastRecID."'";
//echo $query."\n";
		  ...
		  if($result->num_rows > 0)
		  {
			...
			{
				array_push($pointData, array($row["x"], $row["y"], $row["dragging"]));
				if(isset($row["rec_id"]) && $row["rec_id"] > 0)
					$this->users[$ID]->lastRecID = $row["rec_id"];
			}
			
			$result->free();
			$pointData = json_encode($pointData);
			$this->send($user, $pointData);
		  }
      ...
      if (($message = $this->deframe($frame, $user,$headers)) !== FALSE) {
        if ($user->hasSentClose) {
          ...
          if ((preg_match('//u', $message)) || ($headers['opcode']==2)) {
            ...
            $this->stdout("RESAVED:".$message);
			$messageData = json_decode($message);
			$query = "INSERT INTO points (x, y, dragging) VALUES ('".$messageData[0]."', '".$messageData[1]."', '".$messageData[2]."')";
			$mysqli->query($query);
			$user->lastRecID = $mysqli->insert_id;
